/* Yarn AI — studio frontend (Clean & Premium, multi-tenant) */
...
function renderAuth() {
  ...
  <div class="min-h-screen grid lg:grid-cols-2">
    <div class="hidden lg:flex flex-col justify-between p-12 text-white bg-gradient-to-br from-brand-dark via-brand to-[#1aa86a] relative overflow-hidden">
      <div class="flex items-center gap-2.5">
        <div class="w-9 h-9 rounded-xl bg-white/20 grid place-items-center font-extrabold text-lg">Y</div>
        <span class="font-display font-extrabold text-lg">Yarn<span class="opacity-80">AI</span></span>
      </div>
      <div class="relative z-10">
        <h1 class="font-display font-extrabold text-4xl leading-tight">Copy wey sound<br>like your brand.<br>For real Naija.</h1>
        <p class="mt-4 text-white/85 max-w-sm">Pidgin, Yoruba, Igbo, Hausa — tuned to Nigerian consumers, the cultural calendar, and Naira. Not generic AI copy.</p>
        <div class="mt-8 bg-white/12 backdrop-blur rounded-xl2 p-4 max-w-sm border border-white/15">
          <p class="text-sm leading-relaxed">"Aunty, <b>Black Friday don land!</b> 🔥 That Ankara gown wey you dey eye? <b>30% OFF</b> — as e dey hot. <b>Reply YES</b>…"</p>
          <p class="text-[11px] text-white/70 mt-2">— generated for a Lagos fashion brand</p>
        </div>
      </div>
      <p class="text-white/60 text-xs relative z-10">Built with Claude · Naija brand voice engine 🇳🇬</p>
    ...
    <div class="flex items-center justify-center p-6 sm:p-12">
      <div class="w-full max-w-sm">
        <div class="lg:hidden flex items-center gap-2.5 mb-8">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-brand to-[#34d186] grid place-items-center text-white font-extrabold text-lg">Y</div>
          <span class="font-display font-extrabold text-lg">Yarn<span class="text-brand">AI</span></span>
        </div>
        <h2 class="font-display font-extrabold text-2xl">${login ? "Welcome back" : "Create your account"}</h2>
        <p class="text-sm text-muted mt-1">${login ? "Log in to keep yarning." : "Start generating on-brand Naija copy in seconds."}</p>
...
function sidebar() {
  ...
  <aside class="hidden md:flex w-[244px] shrink-0 flex-col bg-white border-r border-line px-3 py-5 sticky top-0 h-screen">
    <div class="flex items-center gap-2.5 px-2 mb-7">
      <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-brand to-[#34d186] grid place-items-center text-white font-extrabold text-lg shadow-sm">Y</div>
      <div><div class="font-display font-extrabold text-[17px] leading-none tracking-tight">Yarn<span class="text-brand">AI</span></div>
        <div class="text-[11px] text-faint leading-none mt-1">Naija voice engine</div></div>
    </div>
    <nav class="space-y-1">
    ...
    <div class="mt-auto space-y-2">
      ...
      <div class="flex items-center gap-2 px-2">
        <div class="w-7 h-7 rounded-full bg-brand-tint text-brand-dark grid place-items-center text-xs font-bold">${esc((state.user?.name||"U")[0].toUpperCase())}</div>
        <div class="min-w-0 flex-1"><div class="text-xs font-semibold truncate">${esc(state.user?.name||"")}</div>
          <div class="text-[10px] text-faint truncate">${esc(state.user?.email||"")}</div></div>
        <button data-logout title="Log out" class="text-faint hover:text-rose-500"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" class="w-4 h-4"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 17l5-5-5-5M21 12H9"/></svg></button>
      </div>
    </div>
  </aside>
  <div class="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white border-t border-line grid grid-cols-5 px-1 py-1.5">
    ${["studio","calendar","brands","favorites","pricing"].map(k=>`
      <button data-nav="${k}" class="flex flex-col items-center gap-0.5 py-1 text-[10px] ${state.view===k?'text-brand':'text-faint'}">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5">${ICON[k]}</svg>
        ${({studio:'Studio',calendar:'Plan',brands:'Brands',favorites:'Saved',pricing:'Plans'})[k]}</button>`).join("")}
  </div>`;
}
...
function topbar() {
  ...
  <header class="bg-white/80 backdrop-blur border-b border-line sticky top-0 z-20">
    <div class="px-5 sm:px-8 h-16 flex items-center max-w-[1180px] mx-auto w-full">
      <div class="md:hidden mr-3 w-8 h-8 rounded-lg bg-gradient-to-br from-brand to-[#34d186] grid place-items-center text-white font-extrabold">Y</div>
      ...
      <button data-nav="pricing" class="ml-auto md:hidden text-xs font-semibold text-brand">${esc(state.usage?.plan_name||"")}</button>
    </div>
  </header>`;
}
      ...
      ${card(`<span class="text-[13px] font-semibold">The brief</span>
        ...
        <div class="flex items-center gap-3 mt-3">
          ...
          <button id="genBtn" ${state.generating?'disabled':''} class="ml-auto inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-brand hover:bg-brand-dark shadow-sm disabled:opacity-50">
            ${state.generating?'<span class="w-4 h-4 border-2 border-white/40 border-t-white rounded-full spin"></span> Yarning…':'✦ Generate'}</button>
        </div>`)}
    </section>
...
function toneChip(t) {
  ...
  return `<button data-tone="${t.key}" title="${esc(t.blurb)}" class="selectable text-left px-3 py-2 rounded-lg border ${on?'border-brand bg-brand-tint ring-1 ring-brand':'border-line bg-paper hover:border-brand/40'}">
    <div class="text-[13px] font-semibold leading-tight">${t.emoji} ${esc(t.label)}</div>
    <div class="text-[10.5px] text-muted leading-tight mt-0.5">${esc(t.blurb)}</div></button>`;
}
function ctChip(c) {
  const on = state.contentType === c.key;
  return `<button data-ct="${c.key}" class="selectable px-2 py-2.5 rounded-lg border text-center ${on?'border-brand bg-brand-tint ring-1 ring-brand':'border-line bg-paper hover:border-brand/40'}">
    <div class="text-lg leading-none">${c.emoji}</div><div class="text-[10.5px] font-medium leading-tight mt-1">${esc(c.label)}</div></button>`;
}
...
function emptyOutput() {
  return `<div class="h-full grid place-items-center text-center py-16"><div class="max-w-xs">
    <div class="text-5xl mb-3">🇳🇬</div><p class="font-display font-bold text-lg">Make we yarn am</p>
  ...
  <div class="space-y-5 pb-24 md:pb-0">
    ${card(`<div class="grid sm:grid-cols-[1.4fr_1fr_.8fr_1.2fr_auto] gap-3 items-end">
      ...
      <button id="calBtn" ${state.calBuilding?'disabled':''} class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-brand hover:bg-brand-dark shadow-sm disabled:opacity-50">
        ${state.calBuilding?'<span class="w-4 h-4 border-2 border-white/40 border-t-white rounded-full spin"></span> Planning…':'✦ Build plan'}</button></div>
...
function calendarLocked() {
  return card(`<div class="text-center py-14"><div class="text-5xl mb-3">🔒</div>
...
function calEmpty() {
  return card(`<div class="text-center py-14"><div class="text-5xl mb-3">🗓️</div>
    <p class="font-display font-bold text-lg">Plan a month in one click</p>
    <p class="text-sm text-muted mt-1 max-w-md mx-auto">Yarn AI builds a full content calendar tuned to that month in Nigeria. Each idea is one click from finished copy.</p></div>`);
}
function calLoading() {
...
function calPostCard(p,i,abbr) {
  ...
  return `<div data-card class="bg-white border border-line rounded-xl2 shadow-card p-4 flex flex-col gap-2.5 transition">
    ...
    <div class="flex items-center gap-1.5 flex-wrap mt-auto pt-1">
      <span class="text-[11px] bg-paper border border-line rounded-md px-1.5 py-0.5">${ct.emoji} ${esc(ct.label)}</span>
      <span class="text-[11px] bg-paper border border-line rounded-md px-1.5 py-0.5">${tone.emoji} ${esc(tone.label)}</span></div>
    <button data-write="${i}" class="mt-1 w-full text-sm font-semibold text-brand-dark bg-brand-tint hover:bg-brand hover:text-white transition rounded-lg py-2">✍ Write this post</button></div>`;
}
...
function brandForm() {
  ...
  return `<div class="max-w-2xl pb-24 md:pb-0">
    <button data-cancel class="text-sm text-muted hover:text-ink mb-3">← Back to brands</button>
    ...
    ${isNew?`<div class="inline-flex bg-paper border border-line rounded-lg p-0.5 mt-2 mb-4 text-sm">
      ...
      <button data-mode="posts" class="px-3 py-1.5 rounded-md ${mode==='posts'?'bg-white shadow-sm font-semibold':'text-muted'}">✨ Learn from my posts</button></div>`:'<p class="text-sm text-muted mb-4">The more you fill in, the more the copy sounds like you.</p>'}
    ${isNew&&mode==='posts'?`
      <div class="bg-white border border-line rounded-xl2 shadow-card p-5">
        <p class="text-sm text-muted mb-2">Paste 2–4 of your best posts/captions. Yarn AI will read your voice and auto-fill the profile — you review, then save.</p>
        ...
        <button id="learnBtn" class="mt-3 inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm text-white bg-brand hover:bg-brand-dark">
          ${state.extracting?'<span class="w-4 h-4 border-2 border-white/40 border-t-white rounded-full spin"></span> Reading your voice…':'✨ Build profile from posts'}</button>
      </div>`:`
...
function favoritesView() {
  ...
  return `<div class="max-w-3xl space-y-3 pb-24 md:pb-0">${items||`<p class="text-sm text-muted">No saved copy yet — hit ⭐ on any generated option.</p>`}</div>`;
}
  ...
  return `<div class="max-w-3xl pb-24 md:pb-0 space-y-5">
    ${card(`<div class="space-y-3.5">
      ...
      <button id="rateBtn" ${state.rateLoading?'disabled':''} class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-brand hover:bg-brand-dark shadow-sm disabled:opacity-50">
        ${state.rateLoading?'<span class="w-4 h-4 border-2 border-white/40 border-t-white rounded-full spin"></span> Working it out…':'💰 What should I charge?'}</button>
      </div>${ADVI_STYLE}`)}
...
function renderRateResult(r) {
  ...
  return `<div class="fade-up space-y-4">
    ...
    ${r.pitch?card(`<div class="text-[13px] font-semibold mb-2">💬 What to tell the client</div><div class="text-sm bg-brand-tint border border-brand/20 rounded-lg p-3 leading-relaxed">${esc(r.pitch)}</div>`):''}
  ...
  return `<div class="max-w-3xl pb-24 md:pb-0 space-y-5">
    ${card(`<div class="space-y-3.5">
      ...
      <button id="brandBtn" ${state.brandLoading?'disabled':''} class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-brand hover:bg-brand-dark shadow-sm disabled:opacity-50">
        ${state.brandLoading?'<span class="w-4 h-4 border-2 border-white/40 border-t-white rounded-full spin"></span> Building strategy…':'🧭 Build my brand strategy'}</button>
      </div>${ADVI_STYLE}`)}
...
function gigsView() {
  ...
  return `<div class="pb-24 md:pb-0 space-y-5">
    ...
    <div class="space-y-2">${state.gigs.length ? state.gigs.map(gigRow).join("") : `<div data-card class="bg-white border border-line rounded-xl2 shadow-card p-8 text-center"><div class="text-4xl mb-2">💼</div><p class="font-display font-bold">No gigs logged yet</p><p class="text-sm text-muted mt-1">Hit "Log a gig" to start tracking what you earn.</p></div>`}</div>
  </div>`;
}
...
async function generate() {
  ...
  finally { state.generating=false; const d=$("#streamDot"); if(d)d.classList.add("hidden"); const btn=$("#genBtn"); if(btn){btn.disabled=false;btn.innerHTML="✦ Generate";} }
}
    ...
    <div class="fade-up bg-paper border border-line rounded-xl2 p-4" style="animation-delay:${i*40}ms" data-cardwrap="${i}">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-1">
          <button data-fb="up" data-i="${i}" title="On-brand — learn from this" class="w-7 h-7 grid place-items-center rounded-lg border border-line bg-white hover:border-brand/40">👍</button>
          <button data-fb="down" data-i="${i}" title="Off-brand" class="w-7 h-7 grid place-items-center rounded-lg border border-line bg-white hover:border-rose-300">👎</button>
          ...
          <button data-star="${i}" class="text-xs px-2.5 py-1 rounded-lg border border-line bg-white hover:border-brand/40">⭐ Save</button>
          <button data-copy="${i}" class="text-xs px-2.5 py-1 rounded-lg border border-line bg-white hover:border-brand/40">Copy</button>
        </div></div>
      ${c.editing
      ...
      <div class="flex flex-wrap items-center gap-1.5 mt-3 pt-2.5 border-t border-line">
        ...
        <button data-refine="${i}" data-instr="__regen" class="text-[11px] px-2 py-1 rounded-md border border-line bg-white hover:border-brand/40">🔄 Fresh version</button>
        ${c.busy?'<span class="w-3.5 h-3.5 border-2 border-brand/30 border-t-brand rounded-full spin"></span>':''}
      </div></div>`).join("");

  $$("[data-copy]",out).forEach(b=>b.onclick=()=>{ navigator.clipboard.writeText(state.cards[+b.dataset.copy].text); b.textContent="Copied ✓"; setTimeout(()=>b.textContent="Copy",1400); });
...
async function sendFeedback(i, rating, btn) {
  try {
    ...
    toast(rating==="up"?"👍 The brand voice will learn from this":"👎 Noted — we'll steer away from this");
  } catch (ex) { handleErr(ex); }
}
